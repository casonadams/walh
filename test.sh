...
if command -v nvim >/dev/null 2>&1; then
  ...
  for theme in "${themes[@]}"; do
    nvim --headless -u NONE -c "source $theme" \
      -c "if !hlexists('MsgSeparator') || !hlexists('Delimiter') || !hlexists('DiagnosticVirtualTextError') || !hlexists('QuickFixLine') || !hlexists('diffAdded') || !hlexists('@punctuation.bracket') || !hlexists('@lsp.type.function') | cquit 1 | endif" \
      -c "qa!"
  done
